...
b: ba"#
    ...
    #[test]
    fn roundtrip_age_json() {
        let file_path = prepare_temp_file(
            "test_roundtrip_age.json",
            b"{
    \"foo\": 2,
    \"bar\": \"baz\"
}",
        ...
        assert!(
            output.status.success(),
            "SOPS failed to encrypt a json file using age"
        );
        let output = Command::new(SOPS_BINARY_PATH)
            .arg("-d")
            .arg(file_path.clone())
        ...
        assert!(
            output.status.success(),
            "SOPS failed to decrypt a json file using age"
        );
    ...
    #[test]
    fn roundtrip_age_binary() {
        let data = b"\"\"{}this_is_binary_data";
        let file_path = prepare_temp_file("test_roundtrip_age.binary", data);
        let output = Command::new(SOPS_BINARY_PATH)
            .arg("-i")
            .arg("-e")
            .arg(file_path.clone())
        ...
        assert!(
            output.status.success(),
            "SOPS failed to encrypt a binary file using age"
        );
        let output = Command::new(SOPS_BINARY_PATH)
            .arg("-d")
            .arg(file_path.clone())
        ...
        assert!(
            output.status.success(),
            "SOPS failed to decrypt a binary file using age"
        );
        assert_eq!(output.stdout, data);
    }
