...
type creationRule struct {
	...
	ACSKMS                  interface{} `yaml:"acs_kms"` // string or []string
...
func extractMasterKeys(group keyGroup) (sops.KeyGroup, error) {
	...
	for _, k := range group.ACSKMS {
		key, err := acskms.NewMasterKey(k.Arn)
		if err != nil {
			return nil, err
		}
		keyGroup = append(keyGroup, key)
...
func getKeyGroupsFromCreationRule(cRule *creationRule, kmsEncryptionContext map[string]*string) ([]sops.KeyGroup, error) {
	...
	if len(cRule.KeyGroups) > 0 {
		...
		acsMasterKeys, err := acskms.MasterKeysFromARNString(strings.Join(acsKmsKeys, ","))
		if err != nil {
			return nil, err
		}
		for _, k := range acsMasterKeys {
			keyGroup = append(keyGroup, k)
		}
