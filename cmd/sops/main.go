...
`
	...
	cmd := &cli.Command{
		Name:      "sops",
		Usage:     "encrypted file editor with AWS KMS, GCP KMS, HuaweiCloud KMS, Azure Key Vault, age, and GPG support",
		ArgsUsage: "sops [options] file",
		Version:   version.Version,
		Authors:   []any{"CNCF Maintainers"},

		UsageText: `sops is an editor of encrypted files that supports AWS KMS, GCP, HuaweiCloud KMS, AZKV,
PGP, and Age
				...
				Action: func(ctx context.Context, c *cli.Command) error {
					...
					for _, item := range tree.Branches[0] {
						if stores.IsComplexValue(item.Value) {
							return cli.Exit(fmt.Errorf("cannot use complex value in environment; offending key %s", item.Key), codes.ErrorGeneric)
						}
						...
						if !ok {
							return cli.Exit(fmt.Errorf("cannot use non-string keys in environment, got %T", item.Key), codes.ErrorGeneric)
						}
						if strings.Contains(key, "=") {
							return cli.Exit(fmt.Errorf("cannot use keys with '=' in environment: %s", key), codes.ErrorGeneric)
						}
...
func getEncryptConfig(c *cli.Command, fileName string, inputStore common.Store, optionalConfig *config.Config) (encryptConfig, error) {
...
func getMasterKeys(c *cli.Command, kmsEncryptionContext map[string]*string, kmsOptionName string, pgpOptionName string, gcpKmsOptionName string, hckmsOptionName string, azureKvOptionName string, hcVaultTransitOptionName string, ageOptionName string) ([]keys.MasterKey, error) {
...
func getRotateOpts(c *cli.Command, fileName string, inputStore common.Store, outputStore common.Store, svcs []keyservice.KeyServiceClient, decryptionOrder []string) (rotateOpts, error) {
	kmsEncryptionContext := kms.ParseKMSContext(c.String("encryption-context"))
...
func toExitError(err error) error {
	if cliErr, ok := err.(cli.ExitCoder); ok && cliErr != nil {
		...
		return cli.Exit(err, execErr.ExitCode())
	} else if err != nil {
		return cli.Exit(err, codes.ErrorGeneric)
	}
	return nil
}

func keyservices(c *cli.Command) (svcs []keyservice.KeyServiceClient) {
...
func outputStore(c *cli.Command, path string) (common.Store, error) {
	...
	return common.DefaultStoreForPathOrFormat(storesConf, path, c.String("output-type")), nil
}
...
func keyGroups(c *cli.Command, file string, optionalConfig *config.Config) ([]sops.KeyGroup, error) {
...
func loadConfig(c *cli.Command, file string, kmsEncryptionContext map[string]*string) (*config.Config, error) {
	var err error
	configPath := c.String("config")
	if configPath == "" {
		// Ignore config not found errors returned from findConfigFile since the config file is not mandatory
...
func shamirThreshold(c *cli.Command, file string, optionalConfig *config.Config) (int, error) {
