...
const (
	...
	// TencentTokenEnvVar is the environment variable name for Tencent Cloud Token.
...
type MasterKey struct {
	...
	secretKey string
	// token is the Tencent Cloud STS token used for authentication.
	token string
}
...
func (key *MasterKey) EncryptContext(ctx context.Context, dataKey []byte) error {
	...
	if err != nil {
		log.WithFields(logrus.Fields{"keyId": key.KeyID}).Info("Encryption failed")
...
func (key *MasterKey) DecryptContext(ctx context.Context) ([]byte, error) {
	...
	client, err := key.createClient()
	if err != nil {
		log.WithFields(logrus.Fields{"keyId": key.KeyID}).Info("Decryption failed")
...
func (key *MasterKey) createClient() (*kms.Client, error) {
	...
	if credential == nil {
		log.WithFields(logrus.Fields{"keyId": key.KeyID}).Info("No valid credentials found")
		return nil, fmt.Errorf("no valid credentials found. Please set TENCENTCLOUD_SECRET_ID and TENCENTCLOUD_SECRET_KEY environment variables")
	}
